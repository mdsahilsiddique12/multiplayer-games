(function() {
    ...
    style.innerHTML = `
        ...
        #gn-settings-trigger svg {
            width: 28px; height: 28px; fill: #ffae00; transition: transform 0.5s ease;
        }
        #gn-settings-trigger:hover { 
            background: #ffae00; box-shadow: 0 0 40px #ffae00; transform: scale(1.1);
        }
        #gn-settings-trigger:hover svg {
            fill: #000; transform: rotate(180deg);
        }
        ...
        #gn-settings-modal {
            ...
            backdrop-filter: blur(12px); animation: modalFadeIn 0.2s ease-out;
        }

        /* SETTINGS BOX */
        .gn-settings-box {
            width: 95%; max-width: 800px; height: 85vh; max-height: 700px;
        ...
        @media (min-width: 768px) { .gn-settings-box { flex-direction: row; } }

        /* SIDEBAR */
        .gn-settings-sidebar {
            width: 100%; background: rgba(0, 0, 0, 0.6);
            border-bottom: 1px solid #333; padding: 20px;
            display: flex; flex-direction: row; gap: 10px; overflow-x: auto;
        }
        @media (min-width: 768px) {
            .gn-settings-sidebar {
                width: 240px; flex-direction: column; border-bottom: none; 
                border-right: 1px solid #333; padding-top: 60px; 
            }
        }
        .gn-tab-btn {
            padding: 12px 20px; color: #64748b; text-align: center;
            font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: 0.9rem;
            letter-spacing: 2px; background: transparent; border: 1px solid transparent; 
        ...
        .gn-tab-btn.active { 
            color: #000; background: #ffae00; 
            box-shadow: 0 0 15px rgba(255, 174, 0, 0.4); border-color: #ffae00;
        }
        ...
        .gn-settings-content::-webkit-scrollbar-thumb { background: #334155; border-radius: 3px; }
        ...
        /* UI ELEMENTS */
        .gn-section-title {
            font-family: 'Orbitron', sans-serif; font-size: 1.4rem; color: #fff;
            margin-bottom: 25px; padding-bottom: 10px; border-bottom: 1px solid rgba(255,255,255,0.1);
            letter-spacing: 2px; text-shadow: 0 0 10px rgba(255,255,255,0.2);
        }
        .gn-profile-header { display: flex; align-items: center; gap: 25px; margin-bottom: 30px; }
        .gn-avatar-circle { 
            width: 80px; height: 80px; border-radius: 50%; border: 2px solid #ffae00; 
            display: flex; align-items: center; justify-content: center; font-size: 2.5rem; 
            background: #000; box-shadow: 0 0 25px rgba(255, 174, 0, 0.15); 
        }
        ...
        .gn-range { 
            width: 100%; height: 6px; background: #1e293b; outline: none; 
            -webkit-appearance: none; border-radius: 3px; margin-top: 10px; 
        }
        .gn-range::-webkit-slider-thumb { 
            -webkit-appearance: none; width: 18px; height: 18px; background: #ffae00; 
            border-radius: 50%; cursor: pointer; box-shadow: 0 0 15px #ffae00; 
        ...
        .gn-logout-btn {
            ...
            cursor: pointer; transition: 0.3s; text-transform: uppercase; border-radius: 8px;
        }
        .gn-logout-btn:hover { 
            background: #ef4444; color: #fff; box-shadow: 0 0 30px rgba(239, 68, 68, 0.4); 
            transform: translateY(-2px);
        }
        .close-settings { 
            position: absolute; top: 20px; right: 25px; font-size: 2rem; color: #64748b; 
            cursor: pointer; z-index: 10; line-height: 0.8;
        }
    ...
    <div id="gn-settings-modal">
        <div class="gn-settings-box">
            ...
            <div class="gn-settings-sidebar">
                <div class="hidden md:block px-6 pb-6 text-xs text-gray-500 font-mono tracking-widest">SYSTEM CONFIG</div>
                <button class="gn-tab-btn active" onclick="switchTab('profile')">Identity</button>
            ...
            <div class="gn-settings-content" id="gn-content-area">
                ...
                <div id="tab-system" class="gn-tab-pane">
                    ...
                    <button class="gn-logout-btn" onclick="triggerLogout()">TERMINATE SESSION</button>
                    <div class="text-center text-[10px] text-gray-700 mt-8 font-mono tracking-widest">GAME NEXUS STUDIOS<br>BUILD: v1.2.0-RC</div>
                </div>
            </div>
        </div>
    </div>
...
window.toggleMuteSetting = function(checkbox) {
    ...
    window.showToast(isMuted ? "Audio Muted" : "Audio Online", isMuted ? "warning" : "success");
};
